...
namespace GJ_LineStation_Change
{
    ...
    public class LineStationContext : DbContext<LineStation>
    {
        ...
        public void updateYBStation(LineStation model,int type)
        {
            string tableExtend = type == 1 ? "@youbu" : "@youbu.kfq";
            //先备份，再修改
            string sql = $"insert into YB_线路拐点表back{tableExtend} select * from YB_线路拐点表{tableExtend} t where t.线路上下行id={model.UDID} and t.站点id={model.stationId}";
            base.excuteSql(sql);
